...
class LinksPages extends Template
{
    ...
    {
        $sel_page = $this->getLayout()->getBlock('cms_page')->getPage()->getId();

        $this->_collection = $this->_linkCollectionFactory->create();
        $this->_collection->join(['selected_pages' =>'menupage'], '(selected_pages.menu_id=main_table.menu_id)and(selected_pages.page_id='.$sel_page.')', 'page_id');
